...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['step']) && $_POST['step'] === 'credentials') {
    ...
    if ($username && $password) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            // Fetch email — students email is in students table, others in users table
            $email = $user['email'] ?? '';
            ...
            if ($sent) {
                ...
                // Clear pending OTP
                foreach (['_otp_code','_otp_expiry','_otp_user_id','_otp_username',
                          '_otp_name','_otp_role','_otp_student_id','_otp_email','_otp_attempts'] as $k) {
                    unset($_SESSION[$k]);
                }
            }
...
<head>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .toggle-pw:hover { color: var(--primary); background: rgba(26,58,92,.06); }

        .login-btn {
            width: 100%; padding: 13px; margin-top: .6rem;
            background: var(--primary); color: white;
        ...
        .login-btn:hover   { background: #14304d; border-color: #14304d; box-shadow: 0 4px 16px rgba(26,58,92,.25); }
        .login-btn:active  { transform: scale(.98); }

        .divider {
            display: flex; align-items: center; gap: 12px; margin: 1.4rem 0 1.2rem;
        }
        .divider::before, .divider::after {
            content: ''; flex: 1; height: 1.5px; background: var(--border); border-radius: 2px;
        }
        .divider span { font-size: .78rem; color: var(--muted); font-weight: 500; white-space: nowrap; }

        .register-link { text-align: center; font-size: .85rem; color: var(--muted); }
        .register-link a { color: var(--primary); font-weight: 700; text-decoration: none; }
        .register-link a:hover { text-decoration: underline; }

        /* ── OTP Step ── */
        .otp-header { text-align: center; margin-bottom: 1.6rem; }
        .otp-icon {
        ...
        .otp-header h2 { font-size: 1.1rem; font-weight: 800; color: var(--primary); }
        .otp-header p  { font-size: .84rem; color: var(--muted); margin-top: 5px; line-height: 1.5; }
        .otp-header strong { color: var(--text); }

        /* 6-box OTP input */
        .otp-boxes {
            display: flex; gap: 10px; justify-content: center; margin: 1.4rem 0;
        }
        .otp-boxes input {
        ...
        #otp_combined { display: none; }

        .resend-row {
            display: flex; align-items: center; justify-content: center;
            gap: 6px; margin-top: .8rem; font-size: .82rem; color: var(--muted);
        }
        .resend-btn {
        ...
        .back-btn {
            ...
            display: block; text-decoration: none; letter-spacing: .2px;
        }
...
<div class="login-page">
    ...
    <div class="login-box">
        ...
        <!-- Back to login -->
        <a href="<?= BASE_URL ?>login.php" class="back-btn" style="margin-top:1rem;"
           onclick="clearOtpSession()">← Back to Login</a>
