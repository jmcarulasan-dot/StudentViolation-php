...
logAudit($conn, 'RECORD_VIOLATION', 'violation', (int)$conn->insert_id, "Student: {$student_no}, Type: {$violation_type}");
...
<body>
    ...
    <div class="svs-app">
        ...
        <div class="confirm-overlay" id="confirmModal">
            <div class="confirm-box">
                ...
                <h3 style="font-size:1rem; font-weight:800; color:var(--primary); margin-bottom:.4rem;">Confirm
                    Violation Entry</h3>
                <p style="font-size:.86rem; color:var(--muted); margin-bottom:1.2rem;" id="confirmText">Are you sure you
                    want to record this violation?</p>
                <div style="background:var(--bg); border-radius:9px; padding:.9rem 1rem; margin-bottom:1.2rem; font-size:.85rem;"
                ...
                <div class="page-wrapper">
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card gold">
                            ...
                            <div class="stat-info">
                                <div class="stat-num"><?= $myToday ?></div>
                        ...
                        <div class="stat-card purple">
                            ...
                            <div class="stat-info">
                                <div class="stat-num"><?= $myWeek ?></div>
                                <div class="stat-label">My Entries This Week</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <?php if ($total > 0): ?>
                        <div class="g2e" style="margin-bottom:1.4rem;">
                            <div class="chart-card">
                                <div class="chart-title"><span class="cdot" style="background:#1a3a5c;"></span>Violations
                                    Trend — Last 6 Months</div>
                                <div class="cwrap"><canvas id="trendChart"></canvas></div>
                            </div>
                            <div class="chart-card">
                                <div class="chart-title"><span class="cdot" style="background:#f0a500;"></span>By Violation
                                    Type</div>
                    ...
                    <!-- Tabs -->
                    <div class="tabs">
                        <button class="tab-btn" id="btn-record" onclick="switchTab('record',this)">📋 Record
                            Violation</button>
                        <button class="tab-btn" id="btn-lookup" onclick="switchTab('lookup',this)">🔍 Student
                            Lookup</button>
                        <button class="tab-btn" id="btn-today" onclick="switchTab('today',this)">
                            📅 Today's Log
                            <?php if ($myToday > 0): ?>
                                <span
                                    style="background:var(--accent);color:#fff;border-radius:10px;padding:1px 7px;font-size:.68rem;margin-left:4px;"><?= $myToday ?></span>
                            <?php endif; ?>
                        </button>
                        <button class="tab-btn" id="btn-all" onclick="switchTab('all',this)">
                            📂 All Violations
                            <span
                                style="background:var(--primary);color:#fff;border-radius:10px;padding:1px 7px;font-size:.68rem;margin-left:4px;"><?= $total ?></span>
                        </button>
                    </div>
                            ...
                            <form id="violationForm" onsubmit="showConfirm(event)">
                                <div class="form-row">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="student_no" id="sno_input" class="form-control"
                                            list="students-list" placeholder="e.g. C26-01-0001-MAN121"
                                            oninput="this.value=this.value.toUpperCase()" required>
                                        <datalist id="students-list">
                                            <?php while ($s = $students->fetch_assoc()): ?>
                                                <option value="<?= htmlspecialchars($s['student_no']) ?>">
                                ...
                                <div class="form-row">
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="date_recorded" id="date_input" class="form-control"
                                            value="<?= date('Y-m-d') ?>" required>
                                    </div>
                                    <div class="form-group">
                                        <label>Description <span
                                                style="color:var(--muted); font-weight:400;">(optional)</span></label>
                                        <input type="text" name="description" id="desc_input" class="form-control"
                    ...
                    <div id="tab-lookup" class="tab-content">
                        <div class="card">
                            ...
                            <p style="font-size:.85rem; color:var(--muted); margin-bottom:1rem;">
                                Look up a student by their ID number to see their violation history and appeal statuses
                                before recording.
                            </p>
                            <form method="GET" style="display:flex; gap:8px; flex-wrap:wrap; align-items:flex-end;">
                                ...
                                <button type="submit" class="btn btn-primary"
                                    style="margin-bottom:1.1rem;">Search</button>
                            </form>
                                ...
                                <?php if ($foundStudent): ?>
                                    <div class="lookup-result">
                                        <div class="lookup-avatar">
                                            <?php if (!empty($foundStudent['profile_photo'])): ?>
                                                <img src="<?= BASE_URL ?>uploads/profile/<?= htmlspecialchars($foundStudent['profile_photo']) ?>"
                                                    alt="Photo">
                                            <?php else: ?>
                                                🎓
                                            <?php endif; ?>
                                        ...
                                        <div class="lookup-info" style="flex:1;">
                                            ...
                                            <span><?= htmlspecialchars($foundStudent['student_no']) ?> ·
                                                <?= htmlspecialchars($foundStudent['course']) ?> Year
                                                <?= $foundStudent['year_level'] ?></span>
                                            <div class="lookup-badges">
                                                <span class="badge" style="background:#e8f0fe; color:#1d4ed8;">
                                                    <?= $foundStudent['vcount'] ?> total
                                                    violation<?= $foundStudent['vcount'] != 1 ? 's' : '' ?>
                                                </span>
                                                <?php if ($foundStudent['vpending'] > 0): ?>
                                                    <span class="badge badge-pending"><?= $foundStudent['vpending'] ?>
                                                        pending</span>
                                                <?php else: ?>
                                                ...
                                                <?php if ($foundStudent['vappeal_pending'] > 0): ?>
                                                    <span class="badge" style="background:#fef3c7; color:#92400e;">
                                                        ⚖️ <?= $foundStudent['vappeal_pending'] ?>
                                                        appeal<?= $foundStudent['vappeal_pending'] > 1 ? 's' : '' ?> pending
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    ...
                                    <?php if ($foundStudent['vappeal_pending'] > 0): ?>
                                        <div class="appeal-warning">
                                            ⚖️ This student has <strong><?= $foundStudent['vappeal_pending'] ?> pending
                                                appeal<?= $foundStudent['vappeal_pending'] > 1 ? 's' : '' ?></strong>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <?php if ($sv['description']): ?>
                                                                        <div style="font-size:.76rem; color:var(--muted);">
                                                                            <?= htmlspecialchars($sv['description']) ?></div>
                                                                    <?php endif; ?>
                                                                </td>
                                                                <td><?= date('M d, Y', strtotime($sv['date_recorded'])) ?></td>
                                                                <td style="font-size:.82rem; color:var(--muted);">
                                                                    <?= htmlspecialchars($sv['recorded_by_name']) ?></td>
                                                                <td><span
                                                                        class="badge badge-<?= $sv['status'] ?>"><?= ucfirst($sv['status']) ?></span>
                                                                </td>
                                                                <td>
                                ...
                                <?php else: ?>
                                    <div class="lookup-result not-found">
                                        ...
                                        <div>
                                            ...
                                            <span style="font-size:.8rem; color:#c53030;">No student with ID
                                                <strong><?= htmlspecialchars($_GET['lookup']) ?></strong> exists in the
                                                system.</span>
                                        </div>
                                    </div>
                                <?php endif; ?>
                    ...
                    <div id="tab-today" class="tab-content">
                        <div class="card">
                            ...
                            <?php else: ?>
                                    ...
                                    <div class="today-entry">
                                        ...
                                        <div style="flex:1;">
                                            <div style="font-size:.88rem; font-weight:700;">
                                                ...
                                                <code
                                                    style="font-size:.76rem; color:var(--primary); font-weight:400; margin-left:6px;"><?= htmlspecialchars($tr['student_no']) ?></code>
                                            </div>
                                            <div style="font-size:.78rem; color:var(--muted);">
                                                <?= htmlspecialchars($tr['violation_type']) ?>
                                                <?php if ($tr['description']): ?> ·
                                                    <?= htmlspecialchars($tr['description']) ?>        <?php endif; ?>
                                            </div>
                                        </div>
                                ...
                                <?php endforeach; ?>
                                <div
                                    style="margin-top:1rem; padding-top:1rem; border-top:1px solid var(--border); font-size:.82rem; color:var(--muted);">
                        ...
                        <div class="card">
                            <div
                                ...
                                <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                                    ...
                                    <input type="date" id="filterFrom" onchange="filterTable()" class="form-control"
                                        style="max-width:150px; margin:0;" title="From date">
                                    <input type="date" id="filterTo" onchange="filterTable()" class="form-control"
                                        style="max-width:150px; margin:0;" title="To date">
                                    <input type="text" id="searchInput" class="form-control" placeholder="🔍 Search..."
                                        oninput="filterTable()" style="max-width:200px; margin:0;">
                                </div>
                            </div>
                            <div style="height:2px; background:var(--border); margin-bottom:1rem; border-radius:2px;">
                            </div>
                            ...
                            <?php else: ?>
                                    ...
                                    <table id="violationsTable">
                                        ...
                                        <tbody>
                                            <?php foreach ($allViolations as $i => $v):
                                                ...
                                                ?>
                                                <tr data-status="<?= $v['status'] ?>" data-date="<?= $v['date_recorded'] ?>">
                                                    <td><?= $i + 1 ?></td>
                                                    <td><code
                                                    ...
                                                    <td style="color:var(--muted); font-size:.83rem;">
                                                        <?= htmlspecialchars($v['description'] ?? '—') ?></td>
                                                    <td><?= date('M d, Y', strtotime($v['date_recorded'])) ?></td>
                                                    <td style="font-size:.82rem; color:var(--muted);">
                                                        <?= htmlspecialchars($v['recorded_by_name']) ?></td>
                                                    <td><span
                                ...
                                <p id="noResults"
                                    style="display:none; text-align:center; color:var(--muted); padding:1rem; font-size:.88rem;">
                                    No results found.</p>
                            <?php endif; ?>
                            <div class="pagination" id="paginationGuard"></div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function filterTable() {
            ...
            guardPage = 1;
            renderGuardPage(matched);
        }

        function renderGuardPage(matched) {
            const rows = [...document.querySelectorAll('#violationsTable tbody tr')];
            const total = matched.length;
            const pages = Math.ceil(total / GUARD_PAGE_SIZE) || 1;
            guardPage = Math.min(guardPage, pages);
            const start = (guardPage - 1) * GUARD_PAGE_SIZE;
            const end = start + GUARD_PAGE_SIZE;
            ...
            buildPagination('paginationGuard', pages, guardPage, (p) => {
                guardPage = p;
                renderGuardPage(matched);
            });
        }
