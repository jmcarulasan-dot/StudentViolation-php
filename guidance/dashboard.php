...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_student') {
        ...
        if ($stmt->execute()) {
            ...
            if (!$error)
                logAudit($conn, 'ADD_STUDENT', 'student', (int) $conn->insert_id, "Student No: {$student_no}, Name: {$name}");
        } else {
    ...
    if ($action === 'update_status') {
        ...
        logAudit($conn, 'UPDATE_VIOLATION_STATUS', 'violation', $vid, "Status changed to: {$status}");
    }
    ...
    if ($action === 'edit_violation') {
        ...
        if (!$error)
            logAudit($conn, 'EDIT_VIOLATION', 'violation', $vid, "Type: {$violation_type}, Status: {$status}");
    }
    ...
    if ($action === 'delete_violation') {
        ...
        logAudit($conn, 'DELETE_VIOLATION', 'violation', $vid, "Violation ID: {$vid}");
    }
    ...
    if ($action === 'delete_student') {
        ...
        logAudit($conn, 'DELETE_STUDENT', 'student', $sid, "Student ID: {$sid}");
    }

    if ($action === 'review_appeal') {
        $vid = intval($_POST['violation_id']);
        $appealDecision = $_POST['appeal_decision'];
        ...
        if (!in_array($appealDecision, ['approved', 'rejected'])) {
            ...
            if (!$error)
                logAudit($conn, 'REVIEW_APPEAL', 'violation', $vid, "Decision: {$appealDecision}, Remarks: {$appealRemarks}");
        }
    }
}
...
foreach ($violations as $v) {
    $label = date('M Y', strtotime($v['date_recorded']));
    if (isset($statusMonthly[$label]))
        $statusMonthly[$label][$v['status']]++;
}
...
if ($_POST) {
    ...
    elseif ($act === 'delete_student')
        $activeTab = 'students';
    elseif ($act === 'review_appeal')
        $activeTab = 'appeals';
    elseif (in_array($act, ['update_status', 'edit_violation', 'delete_violation']))
...
<head>
    ...
    <style>
        ...
        .appeal-card:hover {
            box-shadow: 0 4px 18px rgba(26, 58, 92, .09);
        }
...
<body>
        ...
        <div class="svs-layout" id="svsLayout">
            ...
            <div class="svs-main">
                        ...
                        <form method="POST">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="date_recorded" id="edit_date" class="form-control"
                                        required>
                                </div>
                                <div class="form-group">
                    ...
                    <div class="modal-box" style="max-width:520px;">
                        ...
                        <form method="POST" id="appealReviewForm">
                            ...
                            <div class="form-group" style="margin-top:.9rem;">
                                ...
                                <textarea name="appeal_remarks" id="appeal_remarks_input" class="form-control" rows="3"
                                    required placeholder="Add your reason for approving or rejecting..."></textarea>
                            </div>
                            <div class="modal-actions">
                                <button type="button" class="btn btn-outline"
                                    onclick="closeAppealModal()">Cancel</button>
                ...
                <div class="page-header"
                    ...
                    <a href="<?= BASE_URL ?>guidance/audit_log.php" class="btn btn-outline">📋 Audit Log</a>
                ...
                <div class="tabs">
                    ...
                    <button class="tab-btn" id="btn-analytics" onclick="switchTab('analytics',this)">📈
                        Analytics</button>
                    <button class="tab-btn" id="btn-violations" onclick="switchTab('violations',this)">
                        📂 Violations
                        <span
                            style="background:var(--accent);color:#fff;border-radius:10px;padding:1px 8px;font-size:.68rem;margin-left:5px;"><?= $totalViolations ?></span>
                    </button>
                    <button class="tab-btn" id="btn-appeals" onclick="switchTab('appeals',this)">
                        📋 Appeals
                        <?php if ($pendingAppealsCount > 0): ?>
                            <span
                                style="background:#f0a500;color:#fff;border-radius:10px;padding:1px 8px;font-size:.68rem;margin-left:5px;"><?= $pendingAppealsCount ?></span>
                        <?php endif; ?>
                    </button>
                    <button class="tab-btn" id="btn-students" onclick="switchTab('students',this)">
                        👥 Students
                        <span
                            style="background:var(--primary);color:#fff;border-radius:10px;padding:1px 8px;font-size:.68rem;margin-left:5px;"><?= $totalStudents ?></span>
                    </button>
                    <button class="tab-btn" id="btn-add-student" onclick="switchTab('add-student',this)">➕ Add
                        Student</button>
                </div>
                ...
                <div id="tab-overview" class="tab-content">
                    <div class="mini-row" style="grid-template-columns:repeat(6,1fr);">
                        <div class="mini-card">
                            ...
                            <div>
                                <div class="mini-num"><?= $totalStudents ?></div>
                                <div class="mini-lbl">Students</div>
                            </div>
                        </div>
                        <div class="mini-card gld">
                            <div class="mini-icon" style="background:#fef3c7;">📋</div>
                            <div>
                                <div class="mini-num"><?= $totalViolations ?></div>
                                <div class="mini-lbl">Violations</div>
                            </div>
                        </div>
                        ...
                        <div class="mini-card grn">
                            ...
                            <div>
                                <div class="mini-num"><?= $resolvedCount ?></div>
                                <div class="mini-lbl">Resolved</div>
                            </div>
                        </div>
                        <div class="mini-card pur">
                            <div class="mini-icon" style="background:#ede9fe;">🆕</div>
                            <div>
                                <div class="mini-num"><?= $recentCount ?></div>
                                <div class="mini-lbl">Last 7 Days</div>
                            </div>
                        </div>
                        <div class="mini-card"
                            style="<?= $pendingAppealsCount > 0 ? 'border-color:#fcd34d;background:#fffbeb;' : '' ?>">
                            <div class="mini-icon" style="background:#fef3c7;">📝</div>
                            <div>
                    ...
                    <?php if ($pendingAppealsCount > 0): ?>
                        <div class="alert alert-info" style="margin-bottom:1rem; cursor:pointer;"
                            ...
                            📋 <strong><?= $pendingAppealsCount ?> pending
                                appeal<?= $pendingAppealsCount > 1 ? 's' : '' ?></strong> awaiting your review.
                    ...
                    <div class="g2">
                        <div class="chart-card">
                            <div class="chart-title"><span class="cdot"
                                    style="background:var(--primary);"></span>Violations Trend — Last 6 Months</div>
                            <div class="cwrap"><canvas id="lineChart"></canvas></div>
                        </div>
                        <div class="chart-card">
                            <div class="chart-title"><span class="cdot" style="background:var(--accent);"></span>Pending
                                vs Resolved</div>
                    ...
                    <div class="g2e">
                        <div class="chart-card">
                            <div class="chart-title"><span class="cdot" style="background:var(--accent);"></span>Top 5
                                Students — Most Violations</div>
                            <?php if (empty($topStudents)): ?>
                                <p style="color:var(--muted);font-size:.85rem;text-align:center;padding:2rem 0;">No data
                                    yet.</p>
                            <?php else:
                                ...
                                <?php foreach ($topStudents as $i => $ts): ?>
                                    <div class="top-item">
                                        ...
                                        <div class="top-info">
                                            ...
                                            <div class="bar-mini" style="width:<?= round(($ts['cnt'] / $maxCnt) * 100) ?>%;">
                                            </div>
                                        </div>
                        ...
                        <div class="chart-card">
                            <div class="chart-title"><span class="cdot" style="background:#8b5cf6;"></span>5 Most Recent
                                Violations</div>
                            <?php if (empty($violations)): ?>
                                <p style="color:var(--muted);font-size:.85rem;text-align:center;padding:2rem 0;">None yet.
                                </p>
                            <?php else: ?>
                                <?php foreach (array_slice($violations, 0, 5) as $rv): ?>
                                    <div style="padding:8px 0; border-bottom:1px solid var(--border);">
                                        <div style="display:flex; justify-content:space-between; align-items:center; gap:8px;">
                                            <div>
                                                <div style="font-size:.84rem; font-weight:700;">
                                                    <?= htmlspecialchars($rv['student_name']) ?>
                                                </div>
                                                <div style="font-size:.73rem; color:var(--muted);">
                                                    <?= htmlspecialchars($rv['violation_type']) ?> ·
                                                    <?= date('M d, Y', strtotime($rv['date_recorded'])) ?>
                                                </div>
                                            </div>
                                ...
                                <div style="margin-top:.9rem;">
                                    <button class="btn btn-outline btn-sm"
                                        onclick="switchTab('violations', document.getElementById('btn-violations'))">View
                                        All →</button>
                                </div>
                            <?php endif; ?>
                        </div>
                ...
                <div id="tab-analytics" class="tab-content">
                    <div
                        style="display:flex; gap:10px; flex-wrap:wrap; align-items:center; margin-bottom:1.2rem; background:var(--white); border:1px solid var(--border); border-radius:var(--radius); padding:1rem 1.2rem;">
                        <span style="font-size:.82rem; font-weight:700; color:var(--primary);">Filter Charts:</span>
                        <select id="analyticsCourse" onchange="filterAnalytics()" class="form-control"
                            style="max-width:150px; margin:0; padding:8px 12px;">
                        ...
                        <select id="analyticsYear" onchange="filterAnalytics()" class="form-control"
                        ...
                        <button onclick="resetAnalytics()" class="btn btn-outline btn-sm">✕ Reset</button>
                        <span id="analyticsCount" style="font-size:.8rem; color:var(--muted); margin-left:auto;"></span>
                    </div>
                    <div class="g2e" style="margin-bottom:1rem;">
                        <div class="chart-card">
                            <div class="chart-title"><span class="cdot"
                                    style="background:var(--gold);"></span>Violations by Type</div>
                            <div class="cwrap tall"><canvas id="barChart"></canvas></div>
                        </div>
                        <div class="chart-card">
                            <div class="chart-title"><span class="cdot" style="background:#8b5cf6;"></span>Violations by
                                Course</div>
                            <div class="cwrap tall"><canvas id="courseChart"></canvas></div>
                        </div>
                    </div>
                    <div class="chart-card" style="margin-bottom:1rem;">
                        <div class="chart-title"><span class="cdot" style="background:var(--success);"></span>Pending vs
                            Resolved — Monthly</div>
                                    ...
                                    <tbody>
                                        <?php foreach ($typeData as $type => $cnt):
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div
                                                        style="background:var(--border);border-radius:4px;height:8px;overflow:hidden;">
                                                        <div
                                                            style="background:var(--primary);height:100%;width:<?= $pct ?>%;border-radius:4px;">
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                ...
                <div id="tab-violations" class="tab-content">
                    <div class="card">
                        <div
                            style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;flex-wrap:wrap;gap:.8rem;">
                            <div class="card-title" style="margin:0;border:none;padding:0;">
                                All Violations <span
                                    style="font-size:.8rem;font-weight:600;color:var(--muted);margin-left:6px;">(<?= $totalViolations ?>
                                    total)</span>
                            </div>
                            <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
                                <select id="filterStatus" onchange="filterViolations()" class="form-control"
                                    style="max-width:140px;margin:0;padding:8px 12px;">
                                    <option value="">All Status</option>
                                    <option value="pending">Pending</option>
                                    <option value="resolved">Resolved</option>
                                </select>
                                <select id="filterAppeal" onchange="filterViolations()" class="form-control"
                                    style="max-width:150px;margin:0;padding:8px 12px;">
                                ...
                                <input type="date" id="filterFrom" onchange="filterViolations()" class="form-control"
                                    style="max-width:150px;margin:0;" title="From date">
                                <input type="date" id="filterTo" onchange="filterViolations()" class="form-control"
                                    style="max-width:150px;margin:0;" title="To date">
                                <input type="text" id="searchViolations" class="form-control" placeholder="🔍 Search..."
                                    oninput="filterViolations()" style="max-width:200px;margin:0;">
                            </div>
                        </div>
                        ...
                        <?php else: ?>
                                    ...
                                    <tbody>
                                        <?php foreach ($violations as $i => $v):
                                            ...
                                            <tr data-status="<?= $v['status'] ?>" data-appeal="<?= $appealStatus ?>"
                                                data-date="<?= $v['date_recorded'] ?>">
                                                <td><?= $i + 1 ?></td>
                                                <td><code
                                                        style="font-size:.8rem;color:var(--primary);"><?= htmlspecialchars($v['student_no']) ?></code>
                                                </td>
                                                ...
                                                <td style="font-size:.82rem;color:var(--muted);">
                                                    <?= htmlspecialchars($v['recorded_by_name']) ?>
                                                </td>
                                                <td><span
                                                        class="badge badge-<?= $v['status'] ?>"><?= ucfirst($v['status']) ?></span>
                                                </td>
                                                <td>
                                                    <?php if ($appealStatus === 'pending'): ?>
                                                        <button class="btn btn-sm btn-outline"
                                                            style="border-color:#f0a500;color:#92400e;font-size:.73rem;"
                                                            onclick="openAppealModal(<?= $v['id'] ?>,'<?= addslashes($v['student_name']) ?>','<?= addslashes($v['student_no']) ?>','<?= addslashes($v['violation_type']) ?>','<?= addslashes($v['appeal_text'] ?? '') ?>')">
                                                            📝 Review
                                                        </button>
                                                    <?php else: ?>
                                                ...
                                                <td style="display:flex;gap:4px;flex-wrap:wrap;">
                                                    <button class="btn btn-sm btn-outline"
                                                        onclick="openEdit(<?= $v['id'] ?>,'<?= addslashes($v['violation_type']) ?>','<?= addslashes($v['description'] ?? '') ?>','<?= $v['date_recorded'] ?>','<?= $v['status'] ?>')">Edit</button>
                                                    <form class="action-form" method="POST">
                                                        <input type="hidden" name="action" value="update_status">
                                                        <input type="hidden" name="violation_id" value="<?= $v['id'] ?>">
                                                        <input type="hidden" name="status"
                                                            value="<?= $v['status'] === 'pending' ? 'resolved' : 'pending' ?>">
                                                        <button type="submit"
                                                            class="btn btn-sm <?= $v['status'] === 'pending' ? 'btn-success' : 'btn-outline' ?>">
                            ...
                            <p id="noVResults"
                                style="display:none;text-align:center;color:var(--muted);padding:1rem;font-size:.88rem;">No
                                results found.</p>
                        <?php endif; ?>
                        <div class="pagination" id="paginationGuidance"></div>
                    </div>
                </div>
                    ...
                    <div class="card" style="margin-bottom:1.2rem;">
                        <div class="card-title">
                            ...
                            <?php if (count($pendingAppeals) > 0): ?>
                                <span
                                    style="background:#f0a500;color:#fff;border-radius:10px;padding:2px 9px;font-size:.7rem;margin-left:6px;font-weight:800;"><?= count($pendingAppeals) ?></span>
                            <?php endif; ?>
                        </div>
                        ...
                        <?php else: ?>
                            <?php foreach ($pendingAppeals as $v): ?>
                                <div class="appeal-card urgent">
                                    ...
                                    <div class="appeal-text-box">"<?= nl2br(htmlspecialchars($v['appeal_text'] ?? '—')) ?>"
                                    </div>
                                    <form method="POST">
                                        <input type="hidden" name="action" value="review_appeal">
                                        <input type="hidden" name="violation_id" value="<?= $v['id'] ?>">
                                        <input type="hidden" name="appeal_decision" id="inline_decision_<?= $v['id'] ?>"
                                            value="">
                                        <div class="form-group" style="margin-bottom:.7rem;">
                                            <label style="font-size:.8rem;">Remarks / Notes <span
                                ...
                                <div class="appeal-card">
                                    ...
                                    <?php if (!empty($v['appeal_text'])): ?>
                                        ...
                                        <div class="appeal-text-box" style="margin-bottom:.6rem;">
                                            "<?= nl2br(htmlspecialchars($v['appeal_text'])) ?>"</div>
                                    <?php endif; ?>
                                    <?php if (!empty($v['appeal_remarks'])): ?>
                ...
                <div id="tab-students" class="tab-content">
                    <div class="card">
                        <div
                            ...
                            <div class="card-title" style="margin:0;border:none;padding:0;">
                                All Students <span
                                    style="font-size:.8rem;font-weight:600;color:var(--muted);margin-left:6px;">(<?= $totalStudents ?>
                                    enrolled)</span>
                            </div>
                            <input type="text" id="searchStudents" class="form-control"
                                placeholder="🔍 Search students..."
                                oninput="searchTable('studentsTable','searchStudents','noSResults')"
                                    ...
                                    <tbody>
                                        <?php foreach ($students as $i => $s):
                                            ...
                                            <tr>
                                                <td><?= $i + 1 ?></td>
                                                <td><code
                                                        style="font-size:.8rem;color:var(--primary);"><?= htmlspecialchars($s['student_no']) ?></code>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <?php if ($vc > 0): ?>
                                                        <span class="badge" style="background:#fee2e2;color:#991b1b;"><?= $vc ?>
                                                            violation<?= $vc > 1 ? 's' : '' ?></span>
                                                    <?php else: ?>
                                                ...
                                                <td>
                                                    <form class="action-form" method="POST"
                                                        onsubmit="return confirm('Delete student and ALL violations?')">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="student_no" class="form-control"
                                        placeholder="C26-01-0001-MAN121" required>
                                </div>
                                <div class="form-group">
    ...
    <script>
        ...
        // ── Analytics Filter ──────────────────────────────────────
        const allViolationsData = <?= json_encode(array_map(fn($v) => [
            'course' => $v['course'],
        ...
        let barChartObj, courseChartObj, stackedChartObj;

        function filterAnalytics() {
            const course = document.getElementById('analyticsCourse').value;
            const year = document.getElementById('analyticsYear').value;

            const filtered = allViolationsData.filter(v =>
                (!course || v.course === course) &&
                (!year || String(v.year_level) === year)
            );

            document.getElementById('analyticsCount').textContent =
                filtered.length + ' violation' + (filtered.length !== 1 ? 's' : '') + ' matched';

            // Rebuild type data
            const typeMap = {};
            filtered.forEach(v => { typeMap[v.violation_type] = (typeMap[v.violation_type] || 0) + 1; });
            const tLabels = Object.keys(typeMap).sort((a, b) => typeMap[b] - typeMap[a]);
            const tCounts = tLabels.map(k => typeMap[k]);

            // Rebuild course data
            const courseMap = {};
            filtered.forEach(v => { courseMap[v.course] = (courseMap[v.course] || 0) + 1; });
            const cLabels = Object.keys(courseMap);
            const cCounts = cLabels.map(k => courseMap[k]);

            // Rebuild stacked monthly
            const months = <?= json_encode(array_keys($statusMonthly)) ?>;
            const pendingM = months.map(m => filtered.filter(v => date2label(v.date_recorded) === m && v.status === 'pending').length);
            const resolvedM = months.map(m => filtered.filter(v => date2label(v.date_recorded) === m && v.status === 'resolved').length);

            barChartObj.data.labels = tLabels;
            barChartObj.data.datasets[0].data = tCounts;
            barChartObj.data.datasets[0].backgroundColor = PAL.slice(0, tLabels.length);
            barChartObj.update();

            courseChartObj.data.labels = cLabels;
            courseChartObj.data.datasets[0].data = cCounts;
            courseChartObj.update();

            stackedChartObj.data.datasets[0].data = pendingM;
            stackedChartObj.data.datasets[1].data = resolvedM;
            stackedChartObj.update();
        }

        function date2label(d) {
            const dt = new Date(d);
            return dt.toLocaleString('en-US', { month: 'short' }) + ' ' + dt.getFullYear();
        }

        function resetAnalytics() {
            document.getElementById('analyticsCourse').value = '';
        ...
        courseChartObj = new Chart(document.getElementById('courseChart'), {
            type: 'pie',
        ...
        stackedChartObj = new Chart(document.getElementById('stackedChart'), {
            type: 'bar',
            data: {
        ...
        function filterViolations() {
            ...
            const matched = rows.filter(r => {
                ...
                return r.innerText.toLowerCase().includes(q)
                    ...
                    && (!appeal || r.dataset.appeal === appeal)
                    && (!from || rowDate >= from)
                    && (!to || rowDate <= to);
            });

            guidancePage = 1;
            renderGuidancePage(matched);
        }

        function renderGuidancePage(matched) {
            const rows = [...document.querySelectorAll('#violationsTable tbody tr')];
            const total = matched.length;
            const pages = Math.ceil(total / GUIDANCE_PAGE_SIZE) || 1;
            guidancePage = Math.min(guidancePage, pages);
            const start = (guidancePage - 1) * GUIDANCE_PAGE_SIZE;
            const end = start + GUIDANCE_PAGE_SIZE;

            rows.forEach(r => r.style.display = 'none');
            matched.forEach((r, i) => r.style.display = (i >= start && i < end) ? '' : 'none');

            document.getElementById('noVResults').style.display = total === 0 ? 'block' : 'none';
            buildPagination('paginationGuidance', pages, guidancePage, (p) => {
                guidancePage = p;
                renderGuidancePage(matched);
            });
        }

        function buildPagination(containerId, pages, current, onClick) {
            ...
            html += `<button class="page-btn" ${current === pages ? 'disabled' : ''} onclick="(${onClick.toString()})(${current + 1})">›</button>`;
            html += `<span class="page-info">${current} / ${pages}</span>`;
            el.innerHTML = html;
        }
